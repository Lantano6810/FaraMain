import { Controller, Post, Get, UploadedFile, UseInterceptors, Body, Delete, Param, BadRequestException } from '@nestjs/common';
...
export class MinioController {
    ...
    async uploadFile(
        ...
        if (!bucketName || !serviceId) {
            throw new BadRequestException('Не указано название бакета или service_id.');
        }

        const fileUrl = await this.minioService.uploadFile(bucketName, file.originalname, file.buffer);

        // ✅ Сохраняем фото в базу данных
        const savedPhoto = await this.photoService.savePhoto(serviceId, fileUrl);

        return { message: 'Файл загружен успешно', url: fileUrl, id: savedPhoto.id }; // ✅ Возвращаем id фото
    }
